 ...
 * 
 */
class com_wolfnet_wordpress_settings_dao
extends com_ajmichels_wppf_abstract_dao
implements com_ajmichels_wppf_interface_iDao
{
	...
	{
		$this->log( 'Init com_wolfnet_wordpress_settings_dao' );
	}
	...
	{
		$settingsArray     = array ();
		$settingsPrototype = $this->getEntityPrototype();
		$data              = $this->getData();
		if ( is_array( $data ) && count( $data ) > 0 ) {
			foreach ( $data as $settingsData ) {
				$settings = clone $settingsPrototype;
				$settings->setMemento( $settingsData );
				// Push Object to Array
				array_push($settingsArray, $settings);
			}
