 ...
 * 
 * @title         jquery.wntListingGrid.js
...
( function ( $ ) {
	...
	$.fn.wntListingGrid = function ( options ) {
		...
		return this.each( function () {
			...
			var onResize = function () {
				var itemWidth   = $grid.find( '.mlsFinder_listing:first' ).width();
				var gridWidth   = $grid.width();
				var numColumns  = Math.floor( gridWidth / itemWidth );
				var marginWidth = Math.floor( ( ( ( gridWidth % itemWidth ) - 1 ) / numColumns ) / 2 );
				
				$grid.find( '.mlsFinder_listing' ).css( 'margin-right', marginWidth );
				$grid.find( '.mlsFinder_listing' ).css( 'margin-left',  marginWidth );
