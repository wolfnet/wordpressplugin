...
( function ( $ ) {
	...
	$.fn.wntFeaturedListings = function ( options ) {
		...
		var classPrefix = 'mlsFinder_featuredListings_';
		...
		return this.each( function () {
			...
			{
				var itemWidth      = $featListings.find( '.mlsFinder_listing:first' ).width();
				...
				$featListings.find( '.mlsFinder_listing' ).css( 'margin-right', marginWidth );
				$featListings.find( '.mlsFinder_listing' ).css( 'margin-left',  marginWidth );
			...
			var $innerContainerChildren = $innerContainer.children( 'div.mlsFinder_listing' ).each( function () {
			...
			var move = function ( andContinue, direction, count, speed )
			{
				...
				switch ( direction ) {
					case 'left':
						$innerContainer.find( '.mlsFinder_listing' ).css( 'float', 'left' );
						...
						break;
					case 'right':
						$innerContainer.find( '.mlsFinder_listing' ).css( 'float', 'right' );
			...
			{
				dupChildren = $innerContainer.find( '.mlsFinder_listing' ).slice( 0, num );
			...
			/* AutoPlay
			 * --------
			 * If auto play is enabled initiate scrolling imediately after the plugin is completely
			 * defined. This must be the last code defined in the plugin.
			 */
			if ( option.autoplay ) {
