...
(function ($) {
	...
	MapTracks.prototype.createMap = function (mapNode) {
		...
		if ($(mapNode).is("[data-wnt-map-isMovable]")) {
			params.isMovable = $(mapNode).attr("data-wnt-map-isMovable") === "true";
		} else {
			// Support misspelling
	...
	var MapTracksMap = function (baseNode, params) {
		...
		this.updatePOIStatus = function () {
			...
			if (poiMap.css("display") == "block") {
				poiStatus = 1;
			}

			togglePOIMapStatus = $.ajax({
				type: "GET",
				url: baseURL + "gateway.cfm?action=togglePOIMapStatus&poiStatus=" + poiStatus
			});
		...
		*/
			...
			if ((mapName !== undefined) && (mapName != "poiMiniMap")) {
				...
				window.setTimeout(function () {
					if (myMapTrigger == mapTrigger) {
						$.post(baseURL + "gateway.cfm?action=dbtrack", {
							page_type: page_type,
		...
		// POI Library Load Trigger
		if (mapName == "poi") {
			$(window).bind("loadPOILibraries", function (e) {
				map.loadPOILibraries();
			});
			$(window).bind("updatePOIStatus", function (e) {
				map.updatePOIStatus();
			});
		}
